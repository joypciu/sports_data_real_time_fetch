...
# curl_cffi gives us a Chrome-impersonation session that bypasses Cloudflare.
try:
...
def find_match(
    ...
    away_score_raw, status_type (finished/inprogress/notstarted), finished.
    """
    payload = _get(f"/sport/football/scheduled-events/{game_date}")
    ...
    for ev in events:
        ...
        if team and opponent:
            ...
            score = max(_name_score(opponent, home_name), _name_score(opponent, away_name))
...
def _get_corners(match_id: int) -> dict[str, tuple[int, int]]:
    payload = _get(f"/event/{match_id}/statistics")
