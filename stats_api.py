...
def _fetch_mma_event_espn(
    ...
    for espn_date in dates_to_try:
        ...
        try:
            with _httpx.Client(
                timeout=_ESPN_HTTP_TIMEOUT, follow_redirects=True
            ) as client:
                r = client.get(url)
...
def _basketball_espn_leagues(
    league: str | None = None, sport: str | None = None
) -> list[str]:
    """Ordered ESPN basketball league paths to try for scoreboard/summary."""
    hint = _normalize_text(league or "")
    if hint in {"wnba"}:
        return ["wnba"]
    if hint in {"nba g league", "nba-g-league", "g league"}:
        return ["nba-g-league"]
    if hint in {"ncaab", "ncaa", "mens college basketball", "mens-college-basketball"}:
        return ["mens-college-basketball"]
    if hint in {"nba"}:
        return ["nba"]
    # Generic "basketball" — try NBA then WNBA (covers Fever/Valkyries etc.)
    sport_norm = _normalize_text(sport or "")
    if sport_norm in {"basketball", "nba", ""}:
        return ["nba", "wnba"]
    return ["nba"]
...
def _fetch_basketball_event_espn(
    ...
    for espn_league in _basketball_espn_leagues(league, sport):
        for espn_date in _espn_scoreboard_dates(game_date):
            url = (
                f"https://site.api.espn.com/apis/site/v2/sports/basketball/{espn_league}/scoreboard"
                ...
                for comp in event.get("competitions", []):
                    ...
                    candidate = {
                        ...
                        "status": {"pre": "pre", "in": "in"}.get(
                            comp.get("status", {}).get("type", {}).get("state", "post"),
                            "post",
                        ),
                        "home_team": home_team,
...
def _fetch_espn_nba_summary(
    event_id: str, league: str = "nba"
) -> dict[str, Any] | None:
    import httpx as _httpx
...
def _espn_basketball_prop_settlement(
    *,
    ...
    resolved_league = _basketball_espn_leagues(league, sport)[0]
    if event_id:
        event = {
        ...
        event = _fetch_basketball_event_espn(
            game_date, team, opponent, league=league, sport=sport
        )
    if not event or not event.get("event_id"):
        return None

    resolved_league = str(event.get("league") or resolved_league)
    summary = _fetch_espn_nba_summary(str(event["event_id"]), league=resolved_league)
    if not summary:
        # event_id might be from a different league path — try remaining leagues
        for alt in _basketball_espn_leagues(league, sport):
            if alt == resolved_league:
                continue
            summary = _fetch_espn_nba_summary(str(event["event_id"]), league=alt)
            if summary:
                resolved_league = alt
                break
    if not summary:
    ...
    matched = next(
        (p for p in players if _names_match(player, p["display_name"])), None
    )
    if not matched:
        return {
            "found": False,
...
def _resolve_event(
    ...
    if _normalize_text(sport or "") in {"basketball", "nba", "wnba"}:
        nba_event = _fetch_basketball_event_espn(
            game_date, team, opponent, league=None, sport=sport
        )
        if nba_event:
            return nba_event
...
def _evaluate_market(
    ...
    if market_norm == "moneyline":
        ...
        if ou not in {"over", "under"}:
            # Allow "Seattle Storm Over" style picks
            parts = (pick or "").strip().rsplit(" ", 1)
            if len(parts) == 2 and _normalize_text(parts[1]) in {"over", "under"}:
                hint = hint or parts[0].strip()
                ou = _normalize_text(parts[1])
        if ou not in {"over", "under"}:
            raise HTTPException(
                status_code=400,
                detail="For team_total, pick must be over or under",
            )
        if resolved_line is None:
            raise HTTPException(
                status_code=400,
                detail="line is required for market 'team_total'",
            )
        side = None
        if hint:
            side = _resolve_pick_side(hint, event)
        if side not in {"home", "away"}:
            raise HTTPException(
                status_code=400,
                detail="team_total requires a target team (pass team/player matching home or away).",
            )
        resolved_pick = ou
        if home_score is not None and away_score is not None and not pregame:
            team_pts = float(home_score if side == "home" else away_score)
            if team_pts > resolved_line:
                result = ou == "over"
                outcome = "win" if result else "loss"
            elif team_pts < resolved_line:
                result = ou == "under"
                outcome = "win" if result else "loss"
            else:
                result = None
...
def _resolve_mlb_period_market(market_norm: str) -> tuple[str, tuple | None]:
    import baseball_sofascore_props as _bsofa
    import mlb_period_props as _mlb_period

    market_norm_mlb = _bsofa.normalize_market(market_norm)
    entry = _bsofa.BASEBALL_PROP_STAT_MAP.get(market_norm_mlb)
    if entry and entry[1] in _MLB_PERIOD_MARKET_TYPES:
        return market_norm_mlb, entry
    entry = _mlb_period.PERIOD_PROP_STAT_MAP.get(market_norm_mlb)
    if entry and entry[1] in _MLB_PERIOD_MARKET_TYPES:
        return market_norm_mlb, entry
    return market_norm_mlb, None


def _fetch_mlb_period_prop_result(
    *,
    market_norm_mlb: str,
    game_date: str,
    team: str | None,
    ...
    if not result.get("found"):
        ...
        if "No MLB SofaScore match" in note:
            result = _bsofa.prop_check(
                player=None,
                market=market_norm_mlb,
                game_date=game_date,
...
def _fetch_mlb_player_prop_result(
    *,
    market_norm: str,
    game_date: str,
    team: str | None,
    ...
    import baseball_sofascore_props as _bsofa
    import mlb_period_props as _mlb_period
    import mlb_stats as _mlb_stats
    ...
    if market_norm_mlb in _mlb_period.PERIOD_PROP_STAT_MAP:
        return _mlb_period.prop_check(
            player=player,
            market=market_norm_mlb,
            game_date=game_date,
            team=team,
    ...
    if market_norm_mlb in _mlb_stats.MLB_PROP_STAT_MAP:
        return _mlb_stats.prop_check(
            player=player,
            market=market_norm_mlb,
            game_date=game_date,
            team=team,
...
def _evaluate_mlb_period_market(
    *,
    mlb_market_type: str,
    stat_value: float,
    pick: str,
...
def _finalize_settlement_outcome(
    outcome: str,
...
def stats_market_check(
    ...
    sport_norm = _normalize_text(sport or "")
    market_norm_mlb, mlb_entry = _resolve_mlb_period_market(market_norm)
    mlb_market_type = mlb_entry[1] if mlb_entry else None

    # Baseball period markets: SofaScore first, then mlb_period_props fallback.
    if sport_norm == "baseball" and mlb_entry is not None:
        ...
        result = _fetch_mlb_period_prop_result(
            market_norm_mlb=market_norm_mlb,
        ...
        if result.get("found"):
            ...
            if outcome == "pending" and mlb_market_type not in _MLB_PERIOD_MARKET_TYPES:
                settled = False
            ...
            return JSONResponse(
                ...
                content={
                    ...
                    "outcome": outcome,
                    "settled": settled,
                    "source": result.get("source", "sofascore_baseball"),
                    "game_pk": result.get("game_pk") or result.get("match_id"),
                    "match_id": result.get("match_id") or result.get("game_pk"),
        ...
        if tennis_result.get("found"):
            ...
            return JSONResponse(
                ...
                content={
                    ...
                    "outcome": outcome,
                    "settled": settled,
                    "source": tennis_result.get("source", "sofascore_tennis"),
        ...
        if hockey_result.get("found"):
            ...
            return JSONResponse(
                ...
                content={
                    ...
                    "outcome": outcome,
                    "settled": settled,
                    "source": hockey_result.get("source", "sofascore_hockey"),
            ...
            if not result.get("found"):
                return JSONResponse(
                    ...
                    content={
                        ...
                        "note": result.get(
                            "note", "NBA period lookup did not find data."
                        ),
        ...
        if not result.get("found"):
            return JSONResponse(
                ...
                content={
                    ...
                    "source": result.get("source", "sofascore_baseball"),
                    "note": result.get(
                        "note", "MLB settlement lookup did not find data."
                    ),
                    "game_pk": result.get("game_pk") or result.get("match_id"),
                    "match_id": result.get("match_id") or result.get("game_pk"),
                    "game_status": result.get("game_status"),
    ...
    if event_sport in {"basketball", "nba"} and nba_entry is not None:
        ...
        if not settled:
            ...
            side = _resolve_pick_side(
                pick, event, team_hint=team, opponent_hint=opponent
            )
            if side == "home":
                outcome = (
                    "win"
            ...
            adjusted = (
                (diff + resolved_line) if side == "home" else ((-diff) + resolved_line)
            )
            if adjusted > 0:
                outcome = "win"
                result_bool = True
            ...
            if (had_ot and pick_yes_no == "yes") or (
                (not had_ot) and pick_yes_no == "no"
            ):
                outcome = "win"
                result_bool = True
            else:
            ...
            if first_player and (
                pick_player == first_player
                or pick_player in first_player
                or first_player in pick_player
            ):
                outcome = "win"
                result_bool = True
            else:
        ...
        return JSONResponse(
            ...
            content={
                ...
                "line": resolved_line,
                "player": (
                    result.get("player") if nba_market_type == "player_points" else None
                ),
                "result": result_bool,
                "stat_value": stat_value,
                "outcome": outcome,
    ...
    return JSONResponse(
        _evaluate_market(event, market, pick, line, team_hint=player or team)
    )
...
def stats_prop_check(
    ...
    opponent: Optional[str] = Query(
        None, description="Opposing team (improves match lookup for MLB/SofaScore)"
    ),
    league: Optional[str] = Query(
        None, description="League hint (nba, wnba, …) for ESPN basketball fallback"
    ),
    _: None = Depends(_verify_token),
) -> JSONResponse:
    """
    ...
    # ── 2b. MLB markets (SofaScore first, then period props / MLB Stats API) ─
    import baseball_sofascore_props as _bsofa
    import mlb_period_props as _mlb_period
    import mlb_stats as _mlb_stats

    mlb_markets = (
        set(_bsofa.BASEBALL_PROP_STAT_MAP)
        | set(_mlb_period.PERIOD_PROP_STAT_MAP)
        | set(_mlb_stats.MLB_PROP_STAT_MAP)
    )
    market_norm_mlb = _bsofa.normalize_market(market_norm)

    if market_norm_mlb in mlb_markets:
        if not date:
            raise HTTPException(
                status_code=400,
                detail="Provide date (YYYY-MM-DD) to locate the MLB game.",
            )
        result = _fetch_mlb_player_prop_result(
            market_norm=market_norm,
            game_date=_date_only(date),
            team=team,
            opponent=opponent,
    ...
    if event is None and not event_id and date:
        ...
        for cand in candidate_events:
            ...
            try:
                ...
                probe_rows = []
            if any(
                _player_name_match(player, str(r.get("display_name") or ""))
                for r in probe_rows
            ):
                event = cand
                break
    ...
    if event is None:
        sport_norm = _normalize_text(sport or "")
        if sport_norm in {"basketball", "nba", "wnba"} and date:
    ...
    matched_rows = [
        r for r in rows if _player_name_match(player, str(r.get("player_name") or ""))
    ]
    ...
    if not matched_rows:
        ...
        if sport_norm in {"basketball", "nba", "wnba"} and resolved_event_id:
            espn_result = _espn_basketball_prop_settlement(
                player=player,
                market_norm=market_norm,
                pick_norm=pick_norm,
                ...
                opponent=opponent or event.get("away_team"),
                league=league or event.get("league"),
                sport=sport,
    ...
    if (
        not matched_rows
        and date
        and _normalize_text(sport or "") not in {"basketball", "nba", "wnba"}
    ):
        try:
            candidate_events = _historical_event_candidates(
                None, _date_only(date), sport
